...
DronePhysics::DronePhysics(qreal relative_mass, qreal relative_speed, qreal relative_acceleration, int custom_attribute, QObject *parent) : ObjectFeature(parent)
{
...
void DronePhysics::update()
{
    ...
    if(m_databank->move_action == LEFT){
        qreal temp_dx = m_acceleration * ACCELERATION_CONSTANT * pow(m_databank->deltatime, 2.0)/2.0;
        qreal max_dx= m_speed * SPEED_CONSTANT * m_databank->deltatime * modifier;
        if(abs(temp_dx) > abs(max_dx))
            m_databank->dx -= max_dx;
        else
            m_databank->dx -= temp_dx;
    }
    else if(m_databank->move_action == RIGHT){
        qreal temp_dx = m_acceleration * ACCELERATION_CONSTANT * pow(m_databank->deltatime, 2.0)/2.0;
        qreal max_dx= m_speed * SPEED_CONSTANT * m_databank->deltatime * modifier;
        if(abs(temp_dx) > abs(max_dx))
            m_databank->dx += max_dx;
    ...
    else if(m_databank->move_action == UP){
        qreal temp_dy = m_acceleration * ACCELERATION_CONSTANT * pow(m_databank->deltatime, 2.0)/2.0;
        qreal max_dy= m_speed * SPEED_CONSTANT * m_databank->deltatime * modifier;
        if(abs(temp_dy) > abs(max_dy))
            m_databank->dy -= 5 * max_dy;
        else
            m_databank->dy -= 5 * temp_dy;
    }
    else if(m_databank->move_action == DOWN){
        qreal temp_dy = m_acceleration * ACCELERATION_CONSTANT * pow(m_databank->deltatime, 2.0)/2.0;
        qreal max_dy= m_speed * SPEED_CONSTANT * m_databank->deltatime * modifier;
        if(abs(temp_dy) > abs(max_dy))
            m_databank->dy += max_dy;
        else
            m_databank->dy += temp_dy;
    }
    else if(m_databank->move_action == UP_LEFT){
        {
            qreal temp_dy = m_acceleration * ACCELERATION_CONSTANT * pow(m_databank->deltatime, 2.0)/2.0;
            qreal max_dy= m_speed * SPEED_CONSTANT * m_databank->deltatime * modifier;
            if(abs(temp_dy) > 0.7 * abs(max_dy))
                m_databank->dy -= 0.7 * 5 * max_dy;
            else
                m_databank->dy -= 0.7 * 5 * temp_dy;
        }
        {
            qreal temp_dx = m_acceleration * ACCELERATION_CONSTANT * pow(m_databank->deltatime, 2.0)/2.0;
            qreal max_dx= m_speed * SPEED_CONSTANT * m_databank->deltatime * modifier;
            if(abs(temp_dx) > 0.7 * abs(max_dx))
                m_databank->dx -= 0.7 * max_dx;
            else
                m_databank->dx -= 0.7 * temp_dx;
        }
    }
    else if(m_databank->move_action == UP_RIGHT){
        {
            qreal temp_dy = m_acceleration * ACCELERATION_CONSTANT * pow(m_databank->deltatime, 2.0)/2.0;
            qreal max_dy= m_speed * SPEED_CONSTANT * m_databank->deltatime * modifier;
            if(abs(temp_dy) > 0.7 * abs(max_dy))
    ...
    else if(m_databank->move_action == DOWN_LEFT){
        {
            qreal temp_dy = m_acceleration * ACCELERATION_CONSTANT * pow(m_databank->deltatime, 2.0)/2.0;
            qreal max_dy= m_speed * SPEED_CONSTANT * m_databank->deltatime * modifier;
            if(abs(temp_dy) > 0.7 * abs(max_dy))
                m_databank->dy += 0.7 * max_dy;
            else
                m_databank->dy += 0.7 * temp_dy;
        }
        {
            qreal temp_dx = m_acceleration * ACCELERATION_CONSTANT * pow(m_databank->deltatime, 2.0)/2.0;
            qreal max_dx= m_speed * SPEED_CONSTANT * m_databank->deltatime * modifier;
    ...
    else if(m_databank->move_action == DOWN_RIGHT){
        {
            qreal temp_dy = m_acceleration * ACCELERATION_CONSTANT * pow(m_databank->deltatime, 2.0)/2.0;
            qreal max_dy= m_speed * SPEED_CONSTANT * m_databank->deltatime * modifier;
            if(abs(temp_dy) > 0.7 * abs(max_dy))
...
void DronePhysics::handleInternalsChange()
{
    if(m_owner != nullptr){
